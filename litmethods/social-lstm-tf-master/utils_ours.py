...
class SocialDataLoader():
    ...
    def __init__(self, dtype, batch_size=50, seq_length=5, maxNumPeds=40, dset=1, datasets=range(6), forcePreProcess=False):
        '''
        ...
        if dset == 1:
            self.data_dirs = ['./data/ours/TRAF11/'+dtype, './data/ours/TRAF12/'+dtype,
                              './data/ours/TRAF15/'+dtype, './data/ours/TRAF17/'+dtype,
                              './data/ours/TRAF18/'+dtype, './data/ours/TRAF19/'+dtype]
        elif dset == 2:
            self.data_dirs = ['./data/ours/NGSIM1/'+dtype, './data/ours/NGSIM2/'+dtype,
                              './data/ours/NGSIM3/'+dtype, './data/ours/NGSIM4/'+dtype,
                              './data/ours/NGSIM5/'+dtype, './data/ours/NGSIM6/'+dtype]
        elif dset == 3:
            self.data_dirs = ['./data/ours/Beijing1/'+dtype, './data/ours/Beijing2/'+dtype]
    ...
    def frame_preprocess(self, data_dirs, data_file):
        ...
        for directory in data_dirs:
            ...
            for frame in frameList:
                ...
                if len(pedsList) > self.maxNumPeds:
                    pedsList = pedsList[:self.maxNumPeds]
    ...
    def next_batch(self):
        ...
        while i < self.batch_size:
            ...
            if idx + self.seq_length < frame_data.shape[0]:
                ...
                if len(pedID_list) > self.maxNumPeds:
                    pedID_list = pedID_list[: self.maxNumPeds]

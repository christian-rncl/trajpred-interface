...
class ngsimDataset(Dataset):
    ...
    def __init__(self, mat_file, t_h=30, t_f=50, d_s=1 , enc_size = 64, grid_size = (13,3), upp_grid_size = (7,3)):
        self.D = (scp.loadmat(mat_file)['traj']).astype(float)
    ...
    def getHistory(self,vehId,t,refVehId,dsId):
        if vehId == 0:
            ...
            if self.T.shape[1]<=vehId-1:
                return np.empty([0,2])
            refTrack = (self.T[dsId-1][refVehId-1].transpose()).astype(float)
            vehTrack = (self.T[dsId-1][vehId-1].transpose()).astype(float)
